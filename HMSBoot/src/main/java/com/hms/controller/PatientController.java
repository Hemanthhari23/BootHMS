...
public class PatientController {
	...
	public ResponseEntity<?> getAllPatients(){
		...
		if(patList.size()!=0) {
			...
			return new ResponseEntity<>("No patients data found",HttpStatus.NOT_FOUND);
		}
	}
	@GetMapping("/{id}")
	public ResponseEntity<?> searchPatient(@PathVariable int id){
		Patient obj = service.searchPatient(id);
		if(obj.getId()!=0) {
			service.savePatient(obj);
	...
	public ResponseEntity<?> deletePatient(@PathVariable int id){
		...
		if(patient.getId()!=0) {
			...
			return new ResponseEntity<>("Patient deleted.....",HttpStatus.FOUND);
		}
		else {
			return new ResponseEntity<>("Invalid patient id",HttpStatus.NOT_FOUND);
		}
